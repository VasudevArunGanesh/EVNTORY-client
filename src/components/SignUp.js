...
function SignUp() {
  ...
  let handleSubmit = (event) =>  {
    ...
    const url = "https://evntory-app-api.onrender.com/user/signup";
    axios
      .post(url, obj)
      .then((res) => {
        if (res.status === 200) {
          if (res.data.message === "User exists with email") {
            setMessage("user exists with this email")
  ...
  return ( <div>
    <Navbar pfpicon={false} links={[]} dropdown={[]} buttons={[]} bgcolor={"rgb(34, 34, 34)"} textcolor={"white"} linkto={"../user/login"} username={"LOG IN"} logolink={"/"}/>
    <div className="background-image-sign"></div>
    <div className="container-fluid" >
      <div className="signup-card">
        <div  className="card-title">
          <img className="login-img" src={logo} width="220" height="100" /><h1 className="rowdies-text login-title">Sign Up</h1>
        </div>
        <form onSubmit={handleSubmit} className="was-validated form-login">
          ...
          {/* <div className="invalid-feedback" for="#password">Password is required</div> */}
          <button className="submit-butn" type="submit">SIGN UP</button>
        </form>
